...
class Driver {
	...
	static function showThumbnail($user, $title = NULL)
	{
		...
		if (file_exists(BASE_URI . THUMBS_DIR . strtolower("$user.jpg"))) {
			$img_url = BASE_URL . THUMBS_DIR . strtolower("$user.jpg");
			$img_uri = BASE_URI . THUMBS_DIR . strtolower("$user.jpg");
		} else { // In the case when user has no thumb display default
			$img_url = BASE_URL . DEF_THUMB;
			$img_uri = BASE_URI . DEF_THUMB;
		}
		list($w, $h) = getimagesize($img_uri);
...
<a href="$addr" title="$title">
	<img src="$img_url" alt="$title" title="$title" style="height:{$h}px; width:{$w}px;" />
</a>
EOT;
	}
	...
	static function destroySession() {
		...
		// Clean user cookies
		setcookie('sky-net_user', '', time() - 2592000, '/');
